---
name: obsidian
description: Route Markdown and attachment work through the Obsidian CLI for the Sacredyak vault, and use the vault for Markdown work outside Git repositories. Do not use inside a Git repository.
---
...
| Analyse/review vault note | Low-context file analysis when available; otherwise targeted reads |
| Superpowers doc outside git repo | Hook redirects → use obsidian skill to route correctly |
| Agent system files (`~/.dotfiles/`, `~/.claude/`, `~/.agents/`) | Direct edit, bypass vault routing |

**Key rules:**
- **All vault writes** go through the Obsidian CLI — never write directly with the Write/Edit tool alone
- **Analysis reads** use the harness's low-context file-analysis capability when available; otherwise read only the needed sections
- **Edit reads** must load the current content before computing changes
