...
Produce a repeatable end-of-session summary so the user can `/clear` and start a fresh agent without losing continuity. The audience is a future instance of you, not a stakeholder — this is a context-handoff artifact, not a status report. The next agent should be able to pick up by reading this summary alone.

Also invoke proactively if the user says they're about to `/clear` without having run it yet.
...
1. **Chat output only.** Never write the handoff to a file (not `~/.claude/handoffs/`, not anywhere). Never update memory from this skill.
...
6. **No emojis, no hype, no retrospectives.** Terse and concrete — paths, commands, shell IDs, decisions. This is not a "what went well / what went poorly" retro. Match the tone of a seasoned engineer handing off at end-of-shift.
