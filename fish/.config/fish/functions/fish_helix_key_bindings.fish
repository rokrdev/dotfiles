...
function fish_helix_key_bindings --description 'helix-like key bindings for fish'
    ...
    # fish_vi_key_bindings consumed the shared bindings via eval (they print
    # their body for the caller to run); calling __fish_shared_key_bindings
    # bare merely echoed it. Use the same eval-wrapper as upstream fish.
    function __fish_helix_shared_bindings
        eval "$(__fish_shared_key_bindings)"
    end
    for mode in insert default visual
        __fish_helix_shared_bindings -s -M $mode
    end
    functions -e __fish_helix_shared_bindings
