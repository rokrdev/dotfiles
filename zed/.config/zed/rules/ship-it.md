...
- report remaining `ready`, `paused`, and `blocked` tickets; require confirmation
  before shipping an incomplete feature
...
Tickets completed (N):
  <list from .workflow/kanban/done/>
...
- Ask whether to archive a completed feature with
  `kanban-loop archive <feature>`. Never move or delete ticket files manually.
